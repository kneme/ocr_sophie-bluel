...
const alternateTexts = {
  1: "Abajour suspension bleu turquoise",
...
export function createWorksElements(works) {
  if (works) {
    ...
    for (let i = 0; i < works.length; i++) {
      ...
      imageElement.alt =
        alternateTexts[work.id] ?? "pas de description pour le moment";
...
// Display works in homepage gallery
...
export async function filterWorks() {
  ...
  if (categories && works) {
    ...
    categories.forEach((category) =>
      createFilterBtn(category.name, category.id)
    );
    [...document.querySelectorAll(".filter-btn")].forEach((button) => {
      button.addEventListener("click", function () {
        ...
        } else return displayWorks(createWorksElements(works));
      });
    });
  }
}

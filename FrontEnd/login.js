async function fetchPostLogin(stringifiedBody) {
  const response = await fetch("http://localhost:5678/api/users/login", {
    ...
    console.error("Could not fetch resource (POST /users/login).");
  });
  return response;
}
...
async function adminLogin() {
  ...
  loginForm.addEventListener("submit", function (event) {
    ...
    fetchPostLogin(JSON.stringify(input)).then((resp) =>
      resp.json().then((data) => {
        switch (true) {
          ...
          default:
            return displayMsg("login-failure", "La connexion a échoué");
        }
      })
    );
  });
